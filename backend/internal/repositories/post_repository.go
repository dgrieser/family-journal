...
func (r *Repository) ListPostAttachments(postID int64) ([]models.Attachment, error) {
	...
	query := `SELECT id, post_id, file_name, file_type, file_size, url AS storage_path, created_at FROM attachments WHERE post_id = ?`
...
func (r *Repository) ListAttachmentsForPosts(postIDs []int64) (map[int64][]models.Attachment, error) {
	...
	query, args, err := sqlx.In(`SELECT post_id, id, file_name, file_type, file_size, url AS storage_path, created_at
